...
namespace WpfApp.EntitiesVM
{
    public class SubjectVM : PropertyChangedBase
    {
        ...
        public Нагрузка PlannedLoad { get => Subject.ПлановаяНагрузка; set => Subject.ПлановаяНагрузка = value; }
        public float PlannedLoadSum => PlannedLoad.Lectures + PlannedLoad.Laboratory + PlannedLoad.Practical + PlannedLoad.Test + PlannedLoad.Consultations + PlannedLoad.Exams +
                                       PlannedLoad.Nir + PlannedLoad.CourseDesigning + PlannedLoad.Vkr + PlannedLoad.Hack + PlannedLoad.Hak + PlannedLoad.Rma + PlannedLoad.Rmp;

        public Нагрузка ActualLoad => Convert(Entries);
        public float ActualLoadSum => ActualLoad.Lectures + ActualLoad.Laboratory + ActualLoad.Practical + ActualLoad.Test +
                                      ActualLoad.Consultations + ActualLoad.Exams +
                                      ActualLoad.Nir + ActualLoad.CourseDesigning + ActualLoad.Vkr + ActualLoad.Hack + ActualLoad.Hak +
        ...
        public void UpdateActualLoadSum()
        {
