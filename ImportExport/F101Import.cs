    ...
    public static class F101Import
    {
        ...
        public static List<F101Entry> LoadF101(string path)
        {
            ...
            using (var package = new ExcelPackage(newFile))
            {
                ...
                var row = StartF101Row;
                
                while (!string.IsNullOrWhiteSpace(worksheet.Cells[row, 5].GetValue<string>()))
                {
                    ...
                    var entry = new F101Entry(курс, семестр, номерПотока, имяПотока, дисциплина, кафедра,
